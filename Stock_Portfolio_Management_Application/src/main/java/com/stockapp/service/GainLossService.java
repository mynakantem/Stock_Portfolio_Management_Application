    ...
    public void calculateAndSaveGainLoss(Long portfolioId) {
        ...
        for (Holding holding : holdings) {
            ...
            if (stockPrice != null) {
                ...
                // Calculate gain/loss: (currentPrice - buyPrice) * quantity
                double gainAmount = (currentPrice - buyPrice) * quantity;
                
                // Calculate percentage: (gain / (buyPrice * quantity)) * 100
                double gainPercentage = (gainAmount / (buyPrice * quantity)) * 100;
                
                // Get previous day's gain for daily change calculation
                ...
                if (previousGainLoss != null) {
                    dailyChange = Math.abs(gainAmount - previousGainLoss.getGainAmount());
                } else {
                    dailyChange = Math.abs(gainAmount);
                }
                ...
                if (existingRecord != null) {
                    ...
                    // Create new record
                    GainLoss gainLoss = new GainLoss(portfolioId, holding.getSymbol(), 
                        gainAmount, gainPercentage, dailyChange, today);
